...
class QuantumRouter:
    ...
    async def simulate_molecule(self, api_key: str, payload: dict) -> dict:
        ...
        try:
            ...
            if os.environ.get("QBRIDGE_ALLOW_SIMULATOR_FALLBACK", "").strip().lower() in (
                "1",
                "true",
                "yes",
                "on",
            ):
                from backend.vqe_simulator_fallback import simulate_molecule_fallback

                return simulate_molecule_fallback(
                    payload,
