...
- Electronic structure: prefer PySCF-based integrals; synthetic fallback is optional.
"""
    ...
    *,
    ...
    if smiles and smiles.strip():
        ...
        if trial:
            ...
            if normalize_formula(st) in _GEOMETRIES:
...
def _finalize_problem(
    ...
    *,
    ...
    try:
        ...
        hint = (
            "PySCF is required for electronic-structure integrals (no catalog fallback).\n"
            f"Import error: {e}\n"
            f"Python: {sys.version.split()[0]} on {plat} ({mach})\n"
        )
        if sys.platform == "win32":
            hint += (
                "PySCF does not ship pip wheels for native Windows (including Windows on ARM).\n"
                "Options:\n"
                "  1) Run the API inside WSL2 Ubuntu: pip install -r requirements.txt\n"
                "  2) Run the API via Docker (see Dockerfile.api): docker compose up api\n"
                "  3) Use a Linux/macOS host or x64 Linux VM\n"
            )
        else:
            hint += "Install with: pip install --prefer-binary pyscf>=2.3.0\n"
    ...
    try:
        ...
        raise RuntimeError(
            "qiskit-nature is required. Install with: pip install qiskit-nature"
        ) from e

    atom_str = "; ".join(
        f"{sym} {float(c[0])} {float(c[1])} {float(c[2])}"
        for sym, c in zip(mi.symbols, mi.coords)
    )
    spin = max(0, int(mi.multiplicity) - 1)

    try:
        problem = PySCFDriver(
            atom=atom_str,
            unit=DistanceUnit.ANGSTROM,
            basis=str(meta.get("basis") or "sto3g"),
            charge=int(mi.charge),
            spin=spin,
        ).run()
    except Exception as pyscf_err:
        raise RuntimeError(
            f"PySCF electronic-structure calculation failed: {pyscf_err}"
        ) from pyscf_err

    problem.molecule = mi
    meta["electronic_structure_driver"] = "PySCFDriver"
    meta["windows_fallback"] = False
    meta["windows_fallback_notes"] = ""
